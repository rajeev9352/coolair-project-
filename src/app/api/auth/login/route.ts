...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('Login proxy network error', { code, isAbort, message: fetchErr?.message, reason });
    ...
    if (!res.ok) {
      try {
        if (contentType.includes('application/json')) {
          ...
          console.error('Upstream /api/auth/login error', { status: res.status, errJson });
          return NextResponse.json(errJson, { status: res.status });
        }
        const errText = await res.text();
        console.error('Upstream /api/auth/login error (non-JSON)', { status: res.status, errText });
        return NextResponse.json({ message: errText }, { status: res.status });
      } catch (parseErr: any) {
        console.error('Error parsing upstream error response', {
          status: res.status,
    ...
    // Fallback catch—all unexpected errors
    console.error('Login proxy exception', err);
